...
namespace DAL.Repositories.Generic
{
    public class TableRepository<T> : Repository<T>, ITableRepository<T> where T : BaseEntity
    {
        ...
        /// <summary>
        /// Permanently deletes multiple entities from the database based on a filter (Hard Delete).
        /// </summary>
        public async Task<bool> RemoveRangeAsync(Expression<Func<T, bool>> filter )
        {
            try
            {
                ...
                if (!entities.Any())
                    throw new NotFoundException($"No entities of type {typeof(T).Name} found to delete.", _logger);
            ...
            {
                HandleException(nameof(RemoveRangeAsync),
                    $"Error occurred while hard-deleting multiple entities of type {typeof(T).Name}.", ex);
                throw;
            }
        }
        ...
        /// <summary>
        /// Permanently deletes multiple entities (Hard Delete) by passing the entities directly.
        /// </summary>
